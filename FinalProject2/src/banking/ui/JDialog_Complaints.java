...
import framework.cor.Complain;
import framework.cor.CompliantRecevier;
 ...
 * @author USER
 */
public class JDialog_Complaints extends JDialog {
	...
	javax.swing.JTextField JTextField_To = new javax.swing.JTextField();
         ...
         String[] types={"Select Complaints" ,"Bad Customer Service","Money Fraud","Others"};
   ...
   public JDialog_Complaints(BankFrm parentFrame){
...
//		
		...
		JLabel1.setBounds(20, 20, 90, 24);
                
               
		
                jcombo=new JComboBox<>(boxModel);
                jcombo.setBounds(120,20,150,24);
               // jcombo.setForeground(Color.white);
                jcombo.getEditor().getEditorComponent().setBackground(Color.YELLOW);
                jcombo.setBackground(new Color(95,95,95));
                
                for(String type:types){
                boxModel.addElement(type);
                }
		...
		JTextField_From.setBounds(120, 20, 156, 20);
...
//		
	...
	if(jcombo.getSelectedIndex()>=1){
            level=jcombo.getSelectedIndex();
        }
       String message= textArea.getText();
        
                   Complain compRec=CompliantRecevier.getBosses();
                compRec.companyChain(level, message);
